...
- **Bearer middleware bypasses jwks-cache for the cold path** (major): The cold-path branch in services/auth/middleware.ts calls `new JwksClient()` directly
instead of routing through services/auth/jwks-cache.ts. This violates the no-direct-jwks-fetch
rule and re-introduces the failure mode that caused the 2026-Q1 partial outage. — Recommended action: re-implement.
- **Token refresh edge case at the rolling-window boundary** (minor): A token whose expiry lands exactly at the 5-minute refresh boundary is currently rejected
with 401 instead of being refreshed. Either widen the comparison to `<= 5min` or document
the strict inequality as intentional. — Recommended action: not specified.
