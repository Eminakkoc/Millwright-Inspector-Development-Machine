...
- **No active cycle.** `quest.sh current` exits non-zero. Message: `Refused: no active cycle. Run /mi-init and /mi-run first.`
- **No active feature.** `progress.sh get-active` returns `null`. Message: `Refused: no active feature. Advance to stage 2 first (run /mi-continue or /mi-apply-impact).`
...
Relay the script's stderr and exit code to the inspector verbatim.
...
- Natural-language triggering ("export the context", "build a context bundle", etc.) is **best-effort** — see `docs/bundle/plan.md` §7.2. The slash form `/mi-export-bundle` is the supported invocation. If the millwright cannot match a natural-language request, ask the inspector to type the slash form directly.
