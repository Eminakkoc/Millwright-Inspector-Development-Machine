---
description: Check all millwright-inspector-development-machine dependencies (CLI tools, Python modules, skills, MCP server). Report status and offer to install anything missing after inspector approval.
---

# mi-doctor

**Run this first, before any other `/mi-*` command.** Verifies that every dependency the workflow needs is present on the inspector's machine. If something is missing, the millwright proposes install commands and runs them only after explicit approval.
...
Print a human-friendly summary to the inspector. Group by severity:
...
```
millwright-inspector-development-machine dependency report (os=darwin)
...
The "Manual-testing feature" group records the artifacts and script subcommands that `/mi-manual-test-plan` and `/mi-manual-test-run` depend on (per `docs/manual-testing/plan.md` § 3.8). Detection is in `scripts/doctor.sh`; this command does not duplicate the logic — it only renders the JSON `record` output. Missing items here surface as clear named checks instead of cryptic runtime errors at first invocation. The highest-risk silent-failure is `review.sh:FIELD_RE` — without the `source`/`seed-id` extension, `canonicalize` would corrupt auto-seeded blocks (the new fields would look like freeform paragraphs and break IR_HEAD_RE boundaries).

### Step 3 — If `status == "ok"` — done

> "All required dependencies present. You can run `/mi-run <folder1> [<folder2> ...]` to start the workflow."
...
- **`kind == "plugin"`** — Claude Code plugin or skill (e.g., superpowers brainstorming/writing-plans). **Cannot be installed from Bash** — Claude Code's `/plugin` commands only run inside a session. The millwright renders the hint verbatim and asks the inspector to run the slash commands themselves.
...
Present the proposed commands to the inspector, **grouped into a Bash block and a slash-command block** so the inspector knows which ones the millwright will run vs. which they must run themselves:
...
If there are **only** plugin-kind missing deps and nothing Bash-runnable, skip the `(y/n/select)` prompt — there's nothing for the millwright to execute. Just display the slash-command block and ask the inspector to run them, then re-run `/mi-doctor`.

### Step 5 — Run approved installs

Based on the inspector's response to the Bash-block prompt:
...
When running commands that use `sudo`, surface this explicitly to the inspector and get a second confirmation:
...
- **`status == "ok"`** — tell the inspector:

  > "All required dependencies now present. You can run `/mi-run <folder1> [<folder2> ...]` to start the workflow."

- **`status == "error"` with only plugin-kind deps missing** — the inspector still needs to run the slash commands. Remind them:

  > "Bash installs complete. Still missing plugin-kind deps — please run the slash commands above in this session, then re-run /mi-doctor."

- **`status == "error"` with other kinds still missing** — list what's still missing and let the inspector decide next steps.
...
Only install if the inspector explicitly approves.

## Safety

- **Never install packages without inspector approval.** Every command is shown and confirmed before execution.
- **Never run destructive commands** (`rm`, `reset`, etc.) — doctor only installs.
- **Do not modify shell profiles** (`.bashrc`, `.zshrc`) automatically. If an install hint adds to PATH, prompt the inspector to source the file themselves.
...
- If a dep detection is wrong (e.g., the binary is installed under a different name or in a non-standard location), flag it to the inspector rather than forcing an install. The inspector can set aliases / PATH and re-run `/mi-doctor` to verify.
