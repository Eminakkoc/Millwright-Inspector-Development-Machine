...
```
/mo-manual-test-plan [--from-resume] [--force [--new-seed-family]] [--discard-existing]
```

- `--from-resume` — passed by `/mo-continue`'s Resume Step 7 on auto-fire. Suppresses the duplicate no-existing-plan generation prompt (the overseer already answered `y` to the stage-5 hand-off prompt). If a plan already exists and `--force` was not also passed, the flag uses that plan unchanged and jumps to the "perform now?" prompt — never silently rotates.
...
echo "Existing manual-test plan discarded; manual-test phase marked skipped. Write findings into overseer-review.md and type /mo-continue when done."
exit 0
```
...
The prompt fires only when invoked directly by the overseer and only for a decision that has not already been made. `--from-resume` (auto-fired by `/mo-continue`'s Resume Step 7) suppresses the duplicate **no-existing-plan** generation prompt because the overseer already answered `y` to that same question.
...
- **No existing plan, no `--from-resume`:**
  ...
  - On `n`: `progress.sh set manual-test-state=skipped manual-test-failure-policy=none`. Print `"Skipped. Write findings into overseer-review.md and type /mo-continue when done."` Stop.
...
if ! $CLAUDE_PLUGIN_ROOT/scripts/commits.sh change-summary-fresh "$active_feature"; then
  rc=$?
  case "$rc" in
    1) echo "error: change-summary.md is stale for $active_feature; run /mo-draw-diagrams (or /mo-generate-implementation-diagrams) to regenerate, then re-run /mo-manual-test-plan." >&2 ;;
    2) echo "error: change-summary.md is missing for $active_feature; run /mo-draw-diagrams (or /mo-generate-implementation-diagrams) to regenerate, then re-run /mo-manual-test-plan." >&2 ;;
...
- `## 2. What to run` — per-terminal command set. **Every command line MUST be self-contained** — copy-pasteable into the overseer's terminal without preceding setup. Acceptable shapes (pick one per section, do not mix):
...
- **Change-summary regeneration is owned by `/mo-draw-diagrams`** (or `/mo-generate-implementation-diagrams`). This skill only gates on freshness; it never regenerates change-summary itself.
- **Single-owner discipline for auto-seeding:** this skill does NOT touch `overseer-review.md`. Auto-seeding belongs to `/mo-manual-test-run`. See `docs/manual-testing/plan.md` § 2.2 "Auto-seed ownership recap".
