---
description: Launch a brainstorming review session for the active feature. Reads open findings from inspector-review.md and invokes brainstorming with those findings as work definition. The session runs ISOLATED from mi-workflow — same isolation model as stage 3. The inspector types /mi-continue after the session ends to resume the workflow.
---
...
**Delegation contract.** This command REQUIRES the sub-agents listed below; §8.13's main-read budget forbids main from doing their work itself. **Invoking `/mi-review` IS the user requesting them** — Claude Code's default "do not call the Agent tool unless the user requested it" (and any stricter house rule layered on it) does not reach a sub-agent this command names at the step that names it, so spawn them without asking for extra confirmation. The default still holds everywhere else: never spawn a sub-agent this command does not name, and never invent fan-out to parallelize a step main is supposed to run. If a named delegation genuinely cannot run (type unavailable, harness refusal), say so and stop — never silently do its work in main. Sub-agents: `review-iteration-runner` (Step 3a.2.4 — one fresh sub-agent per brainstorming-mode iteration; direct mode spawns none). Canonical rule: `docs/millwright-inspector-project.md` §8.15.
...
- **Auto-fired** by `/mi-continue`'s Inspector Handler when the inspector types `/mi-continue` after writing findings into `inspector-review.md`.
- **Manually invokable** by the inspector at any point during stage 5 — useful if the inspector wants to start the brainstorming review session before the auto-fire path.

## Preconditions

- `progress.md`'s `active.current-stage` is 5 or 6.
- `inspector-review.md` exists.
- At least one finding in `inspector-review.md` has `status: open`. (If none are open, the workflow auto-completes — see `commands/mi-continue.md` Inspector Step 3a.)
...
[[ -f "$ov_file" ]] || { echo "error: inspector-review.md not found at $ov_file" >&2; exit 1; }
...
- **`## Open findings (snapshot)`** — one line per `IR-NNN` from `review.sh list-open "$active_feature"`, in the order they appear in `inspector-review.md`. Format: `IR-NNN (<severity>): <summary>`.
...
- **`## Decisions`** — folded from `$data_root/workflow-stream/$active_feature/decisions.md` (per `docs/clear-points/plan.md` §5.2 mi-review section, §10 step 4b). Per-iteration review sub-agents read `review-context.md` as their primary context source — this fold-in is the **only path** by which feature-scoped decisions reach them. Without it, a `stage-5-to-6` clear (or any decision written between stages 5 and 6) would silently strand decisions outside the sub-agents' view.

  Same body extraction recipe as `mi-plan-implementation` Step 3.5:
...
### Step 2.6 — Ask the inspector to pick a review mode

Read the suggestion that stage 5 (`/mi-continue` Inspector Step 1.6) persisted to `progress.md`:
...
Wait for the reply. If the inspector presses Enter without typing (accept-default), use the suggestion-derived default (`direct` when suggestion was `direct`, `brainstorming` otherwise). Persist the actual choice:
...
The persisted `active.review-mode` records the inspector's actual choice; `active.review-mode-suggestion` stays as stage 5 set it (it is informational and re-readable for debugging/audit, not mutated by stage 6).

### Step 3a — Brainstorming mode: main-driven loop with fresh per-iteration sub-agents

**Architectural change from prior versions.** Step 3a no longer hands the entire `go again` loop to the brainstorming Skill. Main owns the iteration boundaries. Each iteration spawns one **fresh sub-agent** (`Agent` invocation with `subagent_type: millwright-inspector-development-machine:review-iteration-runner` — explicitly NOT a fork) that addresses the currently-open findings, returns a structured summary, and evaporates. Cascading scopes (`re-spec` / `re-plan`) still happen — but they happen *inside* the sub-agent's context, never accumulating in main.
...
inspector_review="$data_root/workflow-stream/$active_feature/implementation/inspector-review.md"
...
The loop runs while open findings exist AND the inspector has not typed `approve` or `abort`. Each iteration is one full address-pass.
...
If `open_ids` is empty: this should not happen at iteration entry — Step 2 already short-circuits the no-findings case. If it does happen mid-loop (e.g., the inspector manually closed all findings), exit cleanly per Step 3a.2.6 `approve` branch.
...
For each open finding with scope ∈ {`re-spec`, `re-plan`}, read its block in `inspector-review.md` to extract IR-id, summary, and the file paths it references. Compose a small "cascade hints" section that the sub-agent prompt will include — paths and 1-line excerpts ONLY, never file content:
...
If during regeneration you discover the invalidation has a wider blast radius than initially identified, surface that in your return summary's `Findings / risks` so the inspector can decide whether to escalate (e.g., a re-plan finding that turns out to require a re-spec).
```
...
```
You are a fresh sub-agent invoked from `mi-review` Step 3a to address inspector review findings on the "<active_feature>" feature. Your context is isolated from the main session — main does not see your tool calls, only your final return summary.

**Required first reads (in order):**

1. <review_ctx> — compact snapshot of active scope, goals, implemented surface, and open findings. This plus inspector-review.md is enough for most cases.
2. <inspector_review> — canonical source of findings. Authoritative.
...
- `Result: success` — proceed to Step 3a.2.6 (ask inspector).
- `Result: partial` — surface the Findings/risks section to the inspector and ask whether to spawn another sub-agent for the unfinished work or to continue with the current state.
- `Result: blocked` — surface the blockage, ask the inspector how to proceed (often this means dropping into direct mode, or aborting).

##### Step 3a.2.6 — Surface summary to inspector; ask approve / go again / abort
...
>
> Reply:
>   - `approve` — exit the review loop. I'll tell you to type `/mi-continue` to resume mi-workflow and finalize.
>   - `go again` — open `inspector-review.md` (any text editor) to add new findings (plain sentences are fine — I'll canonicalize them; or use the `### IR-NNN` block format directly). Then reply `go again` to start another iteration.
>   - `abort` — invoke `/mi-abort-workflow` to cancel the workflow."

Wait for the reply.

##### Step 3a.2.7 — Branch on reply

- **On `approve`:** Tell the inspector *"Review session approved. Type `/mi-continue` to resume the mi-workflow and finalize."* Exit Step 3a (do NOT call `progress.sh` for stage advance — the Review-Resume Handler in `mi-continue.md` owns finalization).
- **On `go again`:** The inspector may have added free-form findings to `inspector-review.md`. Re-canonicalize:
  ```bash
  ...
  # If spans were found, classify and add them per the Step 1.5 recipe in commands/mi-continue.md.
...
- **On `abort`:** Invoke `/mi-abort-workflow`. Stop.
- **On any other reply:** Treat as a verbal concern. Either (a) ask the inspector to write it into `inspector-review.md` first if it's a substantive review finding (then loop back to Step 3a.2.7's `go again` branch), or (b) address it inline if it's a clarifying question (then re-prompt at Step 3a.2.6).
...
- **Hand-off contract is preserved.** The terminal state is identical to the prior implementation: inspector types `approve`, then `/mi-continue`, and the Review-Resume Handler finalizes. Mi-workflow's state machine doesn't notice the internal refactor.
- **Main runs the iteration boundaries.** The inspector's `approve` / `go again` / `abort` is consumed in main, not inside a Skill primer. This makes the loop reentrant after session breaks: a break between iterations leaves the workflow in a recoverable state (`sub-flow=reviewing` is already set; the next `/mi-continue` falls back into Step 3a's loop logic).
...
`mi-review` does **not** advance past stage 6. The Review-Resume Handler in `/mi-continue` (see `commands/mi-continue.md`) handles the post-session work when the inspector types `/mi-continue` after the session ends: sanity-checking no `open` findings remain, marking `inspector-review-completed=true`, setting `sub-flow=none`, advancing 6 → 7, and auto-firing `/mi-complete-workflow`.

**Do not type `/mi-continue` while a Step 3a sub-agent is mid-iteration** (i.e., while you're waiting on the sub-agent's return summary, or while main is asking for `approve` / `go again` / `abort`). Answer the prompt first; type `/mi-continue` only after Step 3a has fully exited via the `approve` branch and returned control with a "type /mi-continue to resume" message.
...
- The Step 3a sub-agent prompt is a **layered load**: `review-context.md` + `inspector-review.md` are the required first reads; `requirements.md`, `config.md`, `summary.md`, and `primer.md` are on-demand fallbacks the sub-agent reads if a gap surfaces.
- `review-context.md` body is **refreshed each iteration** by Step 3a.2.2 via `review.sh sync-refs --refresh-body` (cache-keyed on `(requirements-id, base-commit, HEAD-at-iteration-start)` — no-op when nothing has moved). The frontmatter `requirements-id` is also synced if `/mi-update-blueprint` ran mid-loop. The sub-agent re-reads canonical files (`inspector-review.md`, `requirements.md`) when current state matters beyond the snapshot.
- `mi-review` does not generate findings. Authoring is the inspector's job (subjective concerns) and brainstorming's job (concerns surfaced during the session, written into `inspector-review.md` directly via the same `review.sh add` interface).
- Brainstorming exits in one of three ways:
  - **Approval (clean exit):** all findings resolved + inspector types `approve` → session ends. The inspector then types `/mi-continue`, which fires the Review-Resume Handler in `/mi-continue` to advance 6→7 and auto-fire `/mi-complete-workflow`.
  - **Abort:** inspector types `/mi-abort-workflow` mid-session → see `commands/mi-abort-workflow.md` for cleanup.
  - **Stuck / dead-end:** brainstorming may exit without addressing all findings. The Review-Resume Handler (triggered when the inspector types `/mi-continue`) detects open findings remaining and prompts the inspector to retry with `/mi-review` or to abort.
...
- There is no iteration cap. Brainstorming controls its own loop; the inspector ends it by typing `approve`.
