...
- **`/mo-update-blueprint` (any regeneration trigger)** — folds scope/constraint entries into the regenerated `requirements.md` Non-goals and `config.md` Overseer Additions. Applies to drift (`spec-update`), `completion`-flavored regenerations, and manual invocations. See §9.6, §10 step 4a.
...
- The status-bar / info-bar (it tracks tokens, not spec content).
...
What this lets us answer at 30 days: did the gate fire as expected, did each offer pair with a resume, and what was the rehydration cost class. Precise clear-taken rates require session-level signals (cache misses, prompt-token deltas) which are out of band for this plan; cross-reference with `usage.log`'s post-clear `context.main` drop (per §6.8) for the strongest available signal.
...
### 6.8 Status bar (`info-bar`) interaction

The `info-bar` feature (see `docs/info-bar/plan.md`) tracks per-stage tokens via a `PostToolUse` hook (`scripts/track-stage-tokens.sh`) that writes a sidecar (`quest/<slug>/.stage-tokens.json`) and an audit log (`quest/<slug>/usage.log`). Clear-points interact with this in five ways:

- **Cumulative tokens are unaffected.** The hook sums tokens across assistant turns from `last_advance_at` to the next stage advance. Tokens spent before a `/clear` are still in the transcript and still get summed; the sidecar's `up_to_that_point_tokens` continues to grow correctly. No double-counting, no understating of cumulative cost.

- **`context.main` becomes the value-tracking signal.** The hook records `context.main = input_tokens + cache_creation_input_tokens + cache_read_input_tokens` from the most recent assistant turn at each stage advance — i.e., main-context occupancy at advance time. After a successful clear-point, the post-clear stage row's `context.main` should drop sharply (only the rehydration set + new turns are in context). This is the metric that *proves* clear-points work, and the one to track in dry-runs (see §7 dry-run #7). The bar itself does NOT display `context.main` today — only `tokens` — so the savings are invisible in the bar but visible in `usage.log`.

- **`/clear` ↔ session_id behavior — VERIFIED.** Per the Claude Code docs and live behavior (claude-code-guide agent verification, see §9.5 resolution): **`/clear` keeps the same `session_id` and continues writing to the same transcript file**. The existing session-break logic in `track-stage-tokens.sh` does NOT misfire on `/clear` — no `session_break` marker is inserted, and pre-clear tokens remain attributed to the prior stage as expected. The hook needs no patch for the happy path.
  - **Known caveat — Claude Code issue [#3046](https://github.com/anthropics/claude-code/issues/3046):** the transcript file path may become temporarily inaccessible to hooks after `/clear` (Stop hooks have been observed to fail on stale paths). If the hook fires on a stage transition immediately post-clear and the transcript is unreadable, the existing failure-mode handling (per `docs/info-bar/plan.md` § "Failure modes") writes a row with `parse_error: true` and `tokens: 0` — graceful degradation, no data corruption. A small future enhancement could distinguish "file unreadable after recent `/clear`" from generic parse errors via a `clear_boundary: true` flag, but is out of scope for the clear-points P0 ship.

- **Hook trigger list does NOT need to change.** The new `progress.sh add-clear-recommendation` helper is an intra-stage state write, not a stage transition; the existing trigger classification correctly treats it as a silent no-op.

- **No status-bar display change in this plan.** The current bar (`up to that point: <Nk> │ current stage: <Mk>`) stays accurate post-clear. Adding a clear-taken marker (e.g., `· stage 3 ↻ ✓`) or a third number for `context.main` is *deferred* per the principle that already excluded sub-context tracking from the bar (`docs/info-bar/plan.md` § "Out of scope"): keep the bar minimal; the richer data lives in the log. Revisit only if dry-runs surface a clear UX need.
...
9. **Dry-run #7 — info-bar interaction (run alongside dry-run #1):** after the post-clear stage-3 row is written by the hook, inspect `quest/<slug>/usage.log` and `.stage-tokens.json` and verify:
   - The stage-3 record is present in the log (no missing record).
   - **No `session_break` marker** was inserted between the stage-2 and stage-3 records. If one appears, the `/clear` ↔ session_id assumption was wrong — patch the hook before shipping (see §6.8 third bullet, §9.5).
   - The stage-3 record's `context.main` is *significantly smaller* than the stage-2 record's `context.main` — this proves the clear cleaned main-context occupancy.
   - The sidecar's `up_to_that_point_tokens` continued monotonically (no reset, no anomaly between the stages).
   - The status bar displays the stage-3 row correctly (no error message, no `?` token values).
10. **Telemetry sanity check:** after 5 real cycles using the gates, inspect `context-ledger.md` and cross-reference with `usage.log`:
    ...
    - **Strongest evidence the gates work — `usage.log`'s `context.main` deltas:** for each `post-offer-resume` row, locate the next stage-advance record in `usage.log` and check its `context.main`. Compared against the pre-gate stage's `context.main`, it should drop substantially. This is the receipt that the clear actually cleaned main; the ledger alone can't prove it.
    - **Qualitative read** on whether overseers feel friction (informal — log subjective notes alongside the cycle).

If 10 shows that `usage.log`'s post-gate `context.main` is not meaningfully smaller than pre-gate AND the rehydration class is `medium`+, the gates are creating cache-bust without actually being used — either the recommendation copy is unclear or the friction outweighs perceived value. Iterate before adding §3.3 (or before shipping it if already in progress).
...
9. ~~Verify `/clear` ↔ session_id behavior~~ **Resolved: same session_id, same transcript file** (per §9.5). No hook patch needed for the happy path. Known caveat — Claude Code issue [#3046](https://github.com/anthropics/claude-code/issues/3046) — surfaces when the transcript file is temporarily unreadable post-clear; existing graceful-degradation handling (`parse_error: true` row) covers this. A future enhancement could distinguish "post-`/clear` parse fail" from generic parse fails via a `clear_boundary: true` flag, but is **not required for the P0 ship**. (~30 min consumed for verification; no patch budget needed.)
10. Dry-runs #1–#5 above plus #7 (info-bar interaction, runs alongside #1). (~3 hours)
11. Ship `stage-2-to-3`, `stage-8-to-2`, **and step 4b's `review-context.md` decisions fold-in** together as P0. (Step 4a's `/mo-update-blueprint` integration ships in the same batch if scheduling allows; otherwise it can land as a fast follow.) Wait 1–2 weeks.
12. Inspect telemetry — both `context-ledger.md` (offer↔resume row pairing, rehydration class) and `usage.log`'s post-gate `context.main` deltas (the strongest signal). Decide on §3.3 secondary gate (`stage-5-to-6`).
13. If shipping §3.3: add the gate at the **top of `commands/mo-review.md`, before Step 2 (state mutation)** (see §3.3, §5.2 mo-review section, §6.5 last bullet). Step 4b's review-context fold-in is already in place from the P0 batch — the gate just plugs into it. Run dry-run #6. (~2 hours, smaller than originally estimated because 4b is no longer part of this step's work.)

Total for primary + boundary gates plus 4b's fold-in: roughly a day-and-a-half of focused work plus dry-run time, plus the step 9 verification. Secondary gate is ~2 hours after telemetry confirms value (4b is already in place).
