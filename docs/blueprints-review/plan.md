...
| D2 | Exact Codex MCP **tool name** (the entry that appears as `mcp__codex__<tool>`) | **Resolved (2026-05-21).** Codex CLI `codex-cli 0.130.0` exposes the `codex` MCP server via stdio entrypoint `codex mcp-server`. The server publishes two tools: `mcp__codex__codex` (main one-shot run tool) and `mcp__codex__codex-reply` (continue an existing session — note: hyphen, not underscore). This design uses **`mcp__codex__codex`** because each review iteration is a stateless one-shot call. The tool name is listed in every sub-agent's `tools:` frontmatter (§6.1, §6.2) and in `scripts/blueprint-review.sh resolve-tool` (§7.2). |
