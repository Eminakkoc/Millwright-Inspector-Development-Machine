...
2. **implementation**: a temporary folder used during implementation and review. Holds:
   ...
   - `grounding-report.md` — stage-2 codebase-grounding snapshot. Written by a fresh sub-agent during `/mo-apply-impact`'s Step A (Codebase-grounding pass) before the requirements body is composed. Carries per-item seam findings, pre-existing components, cycle-flavor classification, and an overall `seam-classification` (backend/frontend/mixed/infra) that drives Step C's optional-structural-diagram decision. Audit/debug state — not blueprint state. Lifecycle matches the rest of `implementation/`: archived alongside at stage 8, deleted on `/mo-abort-workflow`.

   There is no separate pre-implementation diagram set — the chain's plan under `docs/superpowers/plans/` describes intent textually, and the implementation diagrams carry the comparison inside each diagram via the existing-vs-new framing convention. At stage 8 (`mo-complete-workflow`) the whole `implementation/` folder is **archived** into `blueprints/history/v[N+1]/implementation/` (move, not delete) so findings, the review-context snapshot, change-summary, grounding-report, and implementation diagrams are preserved as part of the audit record. `/mo-abort-workflow` deletes the same files (an aborted cycle has no shipped work to archive).
...
- **Stage 1.5 (queue ordering):** delegated cross-feature codebase inspection — but ONLY when journal-only signals are insufficient. Phase 4 of the context-optimization plan establishes a three-step flow: (4a) journal-only proposal from `summary.md` cross-references; (4b) heuristic short-circuit checks `summary.md` for cross-feature mentions or dependency keywords; (4c) only when 4b flags ambiguity, a fresh sub-agent inspects the codebase and writes the body of the active cycle's `queue-rationale.md` (under `quest/<active-slug>/`). Triggers at ≥ 2 features in scope; cached for the cycle via `(cycle-slug, summary-md-hash, HEAD-when-scanned)` so mid-cycle re-entries don't re-derive. The intake-stages-don't-read-code invariant is preserved — main never reads code at stage 1.5.
...
- **Schemas** (one per artifact type): `active-quest`, `change-summary`, `config`, `context-ledger`, `decisions`, `diagrams-readme-blueprint`, `diagrams-readme-implementation`, `grounding-report`, `primer`, `progress`, `queue-rationale`, `reason`, `requirements`, `review-context`, `review-file`, `summary`, `todo-list`. The `progress` schema's `active` block carries the optional drift markers (`drift-check-completed`, `history-baseline-version`), the optional `clear-recommendations` array (within-feature clear-point gates that have already fired — `stage-2-to-3` / `stage-5-to-6`), and the immutable worktree-fingerprint trio (`worktree-path`, `git-common-dir`, `git-worktree-dir`). The `queue-rationale` schema supports the multi-batch shape (`status: draft|confirmed`, `batch: integer`, cumulative `features:`). The two `diagrams-readme-*` schemas validate the sibling READMEs under `blueprints/current/diagrams/` and `implementation/diagrams/` respectively. The `grounding-report` schema validates the stage-2 codebase-grounding snapshot under `implementation/grounding-report.md`. The `context-ledger` schema validates the per-cycle telemetry artifact at `quest/<active-slug>/context-ledger.md` (Phase 6.4). The `decisions` schema validates the feature-scoped verbal-decision log at `workflow-stream/<feature>/decisions.md` (excluded from history rotation; persists for the feature's lifetime — see `docs/clear-points/plan.md` §9.2).
...
- **Templates** (mustache-style, rendered by `frontmatter.sh init`): `active-quest`, `change-summary`, `config`, `context-ledger`, `decisions`, `grounding-report`, `overseer-review`, `primer`, `progress`, `queue-rationale`, `reason`, `requirements`, `review-context`, `summary`, `todo-list` — each pairs with its same-named schema.
...
| 6 → 7 → 8 | Overseer | `/mo-continue` | Review-Resume Handler: check/defer open findings, offer a diagram refresh when review-loop commits exist, then atomically advance 6→7 and auto-fire `/mo-complete-workflow`. |
...
- **Stage 5 — Presented for overseer review**: Covers the millwright announcing stage completion (code + implementation diagrams with existing-vs-new framing) and waiting for the overseer to write findings into `overseer-review.md`. Stage 5 is broader than just findings authoring — it widens to "overseer evaluation," with an optional **manual-test sub-flow** that can run before findings authoring. The Resume Step 7 hand-off prompt asks the overseer whether to generate a manual test plan first; on `y` it auto-fires `/mo-manual-test-plan` (which renders `workflow-stream/[feature]/implementation/manual-test-plan.md` and offers to execute it via `/mo-manual-test-run`). Manual-test artifacts are feature-scoped under `workflow-stream/[feature]/implementation/` (alongside `overseer-review.md`/`review-context.md`/`change-summary.md`) — `manual-test-plan.md`, `manual-test-results.md`, and rotated history under `manual-test-plan.history/` — so the existing archive (`/mo-complete-workflow`) and abort (`/mo-abort-workflow`) plumbing extensions cover them. While a run is in progress, `progress.md` carries `sub-flow=manual-testing` and `manual-test-state=running`; the `/mo-continue` dispatcher routes the `5 | manual-testing` row to a dedicated Manual-Test-Resume Handler that re-enters `/mo-manual-test-run` from the persisted `current-scenario`. After the run finalizes (or when the overseer answers `n` to the manual-test prompt), `sub-flow` returns to `none` and stage 5 continues with findings authoring as before. Completed when the overseer types `/mo-continue` and the millwright confirms `overseer-review.md` exists (empty or populated). See `docs/manual-testing/plan.md` for the full design of the sub-flow.
