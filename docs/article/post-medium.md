...
*A methodology for organizing software development around AI coding agents, where every step produces a reviewable artifact and a human inspector signals when the next step can begin.*

![Millwright-Inspector](./mi-cover.png)
...
**The Millwright (AI agent):**

- Generates the artifacts.
- Advances the workflow on the inspector's approval.

**The Inspector (human):**

- Reviews the artifacts.
- Advances the workflow by approving them.
- Owns the git branch.
- Owns the raw inputs (notes, specs, transcripts).
...
If your team is figuring out its own version of this (what an inspector looks like in your setup, what the artifacts should be, how the relays get signalled), I'd like to hear about it. Responses are open below.
